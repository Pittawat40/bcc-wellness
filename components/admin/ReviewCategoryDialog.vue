      ...
      <div v-if="open" class="cat-overlay" @click.self="emit('close')">
        <div class="cat-dialog">
          <div class="cat-header">
            <h3 class="cat-title"><i class="bi bi-tags"></i> จัดการหมวดหมู่รีวิว</h3>
            <button class="cat-close" @click="emit('close')"><i class="bi bi-x-lg"></i></button>
          </div>
          ...
          <div class="cat-body">
            ...
            <div class="cat-list">
              ...
              <template v-else>
                <div v-if="!categories.length" class="cat-empty">ยังไม่มีหมวดหมู่ กรอกด้านบนเพื่อเพิ่ม</div>
                <div v-for="cat in categories" :key="cat.id" class="cat-row">
                  <template v-if="editingId === cat.id">
                    ...
                    <button class="btn btn-primary btn-sm" @click="saveEdit(cat)" :disabled="!editTh.trim()"><i class="bi bi-floppy"></i></button>
                    <button class="btn btn-ghost btn-sm" @click="editingId = null"><i class="bi bi-x"></i></button>
                  </template>
                  <template v-else>
                    <div class="cat-name">
...
const newTh = ref(""); const newEn = ref("");
const editingId = ref<number | string | null>(null);
const editTh = ref(""); const editEn = ref("");

function addCategory() {
  if (!newTh.value.trim()) return;
  emit("add-category", { name_th: newTh.value.trim(), name_en: newEn.value.trim() });
  newTh.value = ""; newEn.value = "";
}
function startEdit(cat: any) { editingId.value = cat.id; editTh.value = cat.name_th; editEn.value = cat.name_en || ""; }
...
<style scoped>
.cat-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.5); backdrop-filter: blur(4px); z-index: 9999; display: flex; align-items: center; justify-content: center; padding: 20px; }
.cat-dialog { background: #fff; border-radius: 16px; width: 100%; max-width: 560px; max-height: 90vh; display: flex; flex-direction: column; box-shadow: 0 20px 60px rgba(0,0,0,0.2); overflow: hidden; }
.cat-header { display: flex; align-items: center; justify-content: space-between; padding: 20px 24px; border-bottom: 1px solid #e5e7eb; flex-shrink: 0; }
.cat-title { font-size: 1rem; font-weight: 700; color: #111827; margin: 0; display: flex; align-items: center; gap: 8px; }
.cat-close { background: none; border: none; cursor: pointer; color: #6b7280; font-size: 1rem; padding: 4px; border-radius: 6px; display: flex; align-items: center; justify-content: center; }
.cat-close:hover { background: #f3f4f6; color: #111827; }
.cat-body { padding: 20px 24px; overflow-y: auto; flex: 1; }
.add-row { display: flex; gap: 8px; margin-bottom: 16px; align-items: center; }
.add-row .form-input { flex: 1; min-width: 0; }
.cat-list { display: flex; flex-direction: column; gap: 8px; max-height: 320px; overflow-y: auto; }
.cat-empty { text-align: center; padding: 32px 0; color: #9ca3af; font-size: 0.875rem; }
.cat-row { display: flex; align-items: center; gap: 8px; padding: 10px 12px; border: 1px solid #e5e7eb; border-radius: 10px; background: #fafafa; }
...
.cat-footer { padding: 16px 24px; border-top: 1px solid #e5e7eb; display: flex; justify-content: flex-end; flex-shrink: 0; }
