  ...
  <div>
    ...
    <div class="filter-container">
      <div class="search-box">
        ...
        <input v-model="searchQuery" type="text" placeholder="ค้นหาชื่อผู้รีวิว หรือเนื้อหา..." class="search-input" @input="onSearchInput" />
    ...
    <div class="card">
        ...
        <table>
          ...
          <tbody>
            ...
            <tr v-for="(r, index) in items" :key="r.id">
              ...
              <td><div class="cell-primary whitespace-nowrap">{{ r.author_name }}</div></td>
              <td><div class="cell-secondary excerpt">{{ r.content?.slice(0, 60) }}...</div></td>
              <td><span class="stars">{{ "★".repeat(r.rating || 5) }}</span></td>
              <td><span class="tag">{{ categoryLabel(r.category) }}</span></td>
              <td>
                <span :class="['badge', r.status === 'published' ? 'badge-green' : 'badge-gray']">
                  {{ r.status === "published" ? "เผยแพร่" : "ซ่อน" }}
                </span>
              </td>
              <td><div class="cell-secondary whitespace-nowrap">{{ r.created_at?.slice(0, 16).replace("T", " ") }}</div></td>
              <td>
                <div class="action-group">
                  <button class="btn-icon" @click="emit('edit', r)"><i class="bi bi-pencil"></i></button>
                  <button class="btn-icon btn-icon-danger" @click="emit('delete', r.id)"><i class="bi bi-trash3"></i></button>
                </div>
              </td>
            </tr>
          </tbody>
      ...
      <div v-if="totalPages > 1" class="pagination">
        <button class="page-btn" :disabled="page <= 1" @click="goPage(page - 1)"><i class="bi bi-chevron-left"></i></button>
        <button v-for="p in pageNumbers" :key="p" class="page-btn" :class="{ active: p === page }" @click="goPage(p)">{{ p }}</button>
        <button class="page-btn" :disabled="page >= totalPages" @click="goPage(page + 1)"><i class="bi bi-chevron-right"></i></button>
      </div>
    </div>
  </div>
</template>
...
function clearSearch() { searchQuery.value = ""; page.value = 1; emitSearch(); }
function goPage(p: number) { page.value = p; emitSearch(); }
</script>
